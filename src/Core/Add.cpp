...
int main(int argc, char **argv)
{
    ...
    for(std::string file_name : child_files) {
        Blob blob(file_name);
        if (blob.create_blob_bin_data()) {
            std::cerr << "failed to create blob data of " << file_name << std::endl;
            return EXIT_FAILURE;
        };
        if (blob.cleate_obj()) {
            std::cerr << "failed to create blob object of " << file_name << std::endl;
            return EXIT_FAILURE;
        };
        sha1_list.push_back(blob.sha1_str);
    }
